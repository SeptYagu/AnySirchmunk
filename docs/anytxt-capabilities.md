...
本机可执行文件版本信息（当前版本由 `ATGUI.exe` 文件版本、窗口标题和官网 OCR 安装包交叉确认；
上一轮核查记录的是升级前的版本）：

| 组件 | 当前版本 | 上一轮核查版本 |
| --- | --- | --- |
| `ATGUI.exe` | 1.3.3514.0（Windows X86_64 OCR） | 1.3.2477.0 |
| `ATService.exe` 等其他组件 | 与 `ATGUI.exe` 同一次安装 | 1.3.2477.0 |

接口基线是升级后的 v1 API：`http://127.0.0.1:9924/rpc`（方法族 `anytxt.v1.*`）。
升级前核查的是旧接口 `http://127.0.0.1:9920`（`ATRpcServer.Searcher.V1.*`）；该接口在 OCR 1.3.3514 上
...
官网 Windows 64 位下载项是两个独立安装包：普通版为
[`Anytxt_1.3.3541_Windows_X86_64.exe`](https://sourceforge.net/projects/anytxt/files/Anytxt_1.3.3541_Windows_X86_64.exe/download)，
OCR 版为
[`Anytxt_1.3.3514_Windows_X86_64_OCR.exe`](https://sourceforge.net/projects/anytxt/files/Anytxt_1.3.3514_Windows_X86_64_OCR.exe/download)。
本机安装的是后者，所以 1.3.3514 是当前官网 OCR 基线，不是低于 1.3.3541 的待升级旧版。项目不能依赖
只在其他 edition 或未验证版本中出现的能力；Sirchmunk 仍根据返回路径读取原文件。

## 4. 已验证的 RPC

本节记录**当前接口基线**，即 AnyTXT Windows X86_64 OCR 1.3.3514 的 v1 API（`anytxt.v1.*`，`127.0.0.1:9924/rpc`）。
...
2026-09-15 复核确认（1.3.2477 与 OCR 1.3.3514 一致），本地服务的请求必须同时满足两个条件：
...
### 4.5 升级到 Windows X86_64 OCR 1.3.3514 之后的实测（2026-09-15）
...
升级到 Windows X86_64 OCR 1.3.3514 后对 v1 端点重做同一对照：`filterDir` 传 `""` 仍回显 `"C:"`（只返回 C 盘），
