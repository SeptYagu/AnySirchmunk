...
只使用 AnyTXT 1.3.3541+ 的 v1 方法，参数直接位于 `params`。候选发现：
...
1.3.3541 实测均为 `C:`），只返回该卷的结果。当前实现先尊重非空 `ANYTXT_GLOBAL_ROOTS`；否则在
...
2. 使用已验证的 1.3.3541 v1 配置；空 `filterDir` 只表示服务端默认目录，未知版本不凭零结果自动切换到 `"*"`。
