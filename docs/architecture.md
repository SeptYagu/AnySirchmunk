...
只使用已在 AnyTXT Windows X86_64 OCR 1.3.3514 上验证的 v1 方法，参数直接位于 `params`。候选发现：

```json
{
  "jsonrpc": "2.0",
  "method": "anytxt.v1.getResult",
  "params": {
    "pattern": "搜索词",
    "filterDir": "C:\\",
    "filterExt": "",
...
`order` 取 3（路径升序）而不是默认值 0：默认排序不保证跨页稳定，长枚举会在翻页时漏掉或重复文件。

响应会给出字段描述、本页行数和文件记录，当前验证到的字段为 `fid`、`lastModify`、`size` 和 `file`。
其中 `count` 是**本页**行数，不能当总数用；精确总数由 `anytxt.v1.search` 单独取得。
...
{
  "jsonrpc": "2.0",
  "method": "anytxt.v1.getFragment",
  "params": {"fid": "搜索结果中的文件标识（十进制字符串）", "pattern": "搜索词"}
}
```
...
{
  "jsonrpc": "2.0",
  "method": "anytxt.v1.status",
  "params": {}
}
```

匹配文本从响应的 `output.text` 读取，命中词被 `*<<*`/`*>>*` 包裹——该标记属于展示层，在转换为证据前必须剥离。
`anytxt.v1.status` 的 `output.return` 表示引擎是否加载完成，用于真正的就绪检查：服务在但引擎未就绪时，
空结果与"无命中"必须能区分开。

请求必须使用 JSON-RPC 2.0 信封并同时带 `Accept: application/json` 和 `Content-Type: application/json`，
缺少任一头部时服务端返回 `HTTP 400`。v1 把业务状态放在 `result.errno`：`filterDir` 指向未索引卷、
`fid` 无法解析时返回 `errno = 1` 与空载荷，而参数错误走 JSON-RPC `error`（`-32602`）。因此
`errno` 不能与"成功的空结果"混为一谈：该目录会被标记为 `scope_errno_<n>` 且结果不完整，但其他目录
已发现的候选必须保留。

RPC 地址、请求限制和超时都由配置提供。用户明确指定一个或多个根目录时，每个根目录分别请求，结果再按规范化后的绝对路径去重。

2026-09-15 复核发现空 `filterDir` 并不是全局搜索：服务端把它解析为自己的当前目录（1.3.2477 与
Windows X86_64 OCR 1.3.3514 实测均为 `C:`），只返回该卷的结果。当前实现先尊重非空 `ANYTXT_GLOBAL_ROOTS`；否则在
Windows 上枚举固定盘，并对每个根发送不可能命中的只读 `anytxt.v1.search` 探针：`errno=0` 表示该盘
可由索引检索，非零则排除。发现结果按检索器实例缓存，不扫描文件、不探测移动盘或网络盘，也不修改索引。
若关闭自动发现且未配置根目录，才退化为空 `filterDir`，并标记 `unverified_global_scope`。
本机 `"*"` 返回零结果，官方论坛示例不能硬编码。详细记录见
[anytxt-capabilities.md](anytxt-capabilities.md)。
...
| `path` | 未明确限定时优先按 `ANYTXT_GLOBAL_ROOTS`，否则自动探测已索引固定盘，逐根查询并合并。明确限定时每个根目录对应一次请求 |
| `case_sensitive` | 仅在原生语义已验证或完整文本可等价验证时支持；否则按范围契约回退或报不支持 |
| `literal` | 单词按字面子串查询；多词短语先用 OR 表达式扩展候选，再用 `getText` 验证大小写不敏感的精确子串。含 `&|!()"` 的词在转义规则未验证前拒绝 |
...
先确认上游 `logic` 是文件级还是行级语义；文件集合运算不能替代行级匹配。对于已确认的文件级语义，OR 可返回标记为不完整的候选并集；AND 和 NOT 必须取得所有相关查询的完整集合后运算。集合不完整时返回 `incomplete_results`，按范围契约回退或报告无法准确执行，不将截断集合视为完整结果。多词 literal 的 `getText` 验证只决定候选是否包含目标子串；截断文本中未找到短语不能证明不命中，因此返回安全子集并标记 `text_validation_truncated`。最终证据仍由 Sirchmunk 读取原文件。

### 4.4A 分页与完整性

- `ANYTXT_SEARCH_LIMIT` 是页大小，不是总候选上限；初始兼容值为 300。实现前验证 `offset`、排序和 `count` 的实际语义。
- 分页使用已验证的稳定排序，按实际收到的记录数推进 offset，并按规范化路径去重。精确总数开启时按每个 scope 的唯一 `fid` 数量对账，而不是按包含重复项的原始行数；跨页部分重叠、唯一数超出或不足总数均标记不完整。只有已验证的结束条件（短页、空页或可信总数）满足，且没有记录丢弃或已知索引变化，才标记完整；完整性仅相对于本次索引查询，不承诺磁盘资料全部已入索引。
...
AnyTXT 1.3.2477 的图形界面和本地资源明确提供正则搜索，本机 RPC 对若干正则形式也能返回结果，但公开的 `anytxt.v1.getResult` 输入没有独立 `searchType` 字段。适配器需要以受控查询验证当前 RPC 状态，不能假定 GUI 选项与 RPC 始终同步。
...
2. 使用已验证的 Windows X86_64 OCR 1.3.3514 v1 配置；空 `filterDir` 只表示服务端默认目录，未知版本不凭零结果自动切换到 `"*"`。
...
`ANYTXT_FALLBACK_ROOTS` 是绝对目录组成的 JSON 数组，默认 `[]`；无效路径或配置应报配置错误，不替换为当前目录或所有磁盘。`ANYTXT_GLOBAL_ROOTS` 同为绝对目录 JSON 数组、默认 `[]`，非空时覆盖自动发现；`ANYTXT_AUTO_DISCOVER_ROOTS` 默认 `true`，只读探测固定盘。它们只决定 AnyTXT 检索范围，不改变回退目录，也不改变 `ANYTXT_FALLBACK_TO_RGA` 的作用范围。全局模式使用文件名枚举但没有回退根目录时，报告 `scope_required`；可选的 FAST 文件名步骤跳过并记录原因。所有内容检索回退均受 `ANYTXT_FALLBACK_TO_RGA` 开关控制，文件名枚举不受该开关控制。
...
- 正常搜索响应转换为完整的事件组。
- 显式目录搜索、显式全局根、固定盘自动发现、发现缓存以及关闭发现后的降级分别按预期工作。
- 请求必须使用 JSON-RPC 2.0 信封并同时带 `Accept` 与 `Content-Type`；改动信封结构或删减任一头部视为契约回归。
- 关闭自动发现且未配置 `ANYTXT_GLOBAL_ROOTS` 时结果标记 `unverified_global_scope`、`complete=false`；该状态下 AND/NOT 不得返回精确结果。
...
- 不配置 `ANYTXT_GLOBAL_ROOTS`，核对自动发现只保留可检索固定盘并能跨卷返回候选；再关闭自动发现，确认结果被标记为不完整而不是全局结果。
...
记录候选 recall@K（相同 K）、检索阶段 P50/P95、RPC 次数、片段字符数、完整性、回退率；LLM 答案总时延另列。发布门槛：共同支持的查询集上 AnyTXT 平均 recall@K 不低于 rga，热缓存检索 P95 不高于 rga 的 80%；精确语义测试全部通过。

2026-09-16 的 49 份公开 PDF / 30 查询实测已通过：冷运行各 30 次，热运行各 150 次；AnyTXT 与 rga 的平均 recall@10 均为 1.0，AnyTXT 热 P95 1366.56 ms、rga 热 P95 3944.27 ms（34.6%），AnyTXT 完整率 100%、回退率 0%。完整可复现结果位于 `benchmarks/results/2026-09-16-jsbach-30-final.json`。precision 与负例差异同时记录，但 rga 提取结果只是对照基准，不视作完美人工真值；AnyTXT 索引文本和 rga 临时提取的规范化差异需要单独解释。

预算分为候选发现与片段两部分：候选发现默认单次 RPC 15 秒、整次 retrieve 30 秒、并发 2、总 RPC 100、候选文件 3000；片段默认 100 次请求、片段总量 100000 字符。片段预算独立，避免大量片段请求把候选发现的分页预算耗尽，两者共享同一个截止时间。任一预算耗尽都只把结果标记为不完整（`complete=false` 与对应 reason），已发现的候选必须保留，不得丢弃候选或向调用方抛出预算异常；取消仍然向上传递，预算耗尽也不启动回退。就绪检查（`status`）、精确计数（`search`）、所有关键词与目录、分页、片段及回退均计入适用预算；每次 RPC 超时取配置的单次超时与剩余预算较小值，调用方 timeout 与配置总预算取较小值。预算值均可配置，名称与默认值见需求 FR-8；进程级 `_EndpointGate` 按 API 端点跨客户端共享，同时限制同类 RPC 并发并保证 `getResult` 与 `getFragment` 不会同时处于实际 HTTP 请求中。单次 retrieve 的预算不代表整个多轮 DEEP 查询预算，外层取消或截止时间也须向下传递。线程包装 HTTP 时取消不能强制终止已运行线程，因此闸门由工作线程持有到传输实际结束；排队线程收到取消标志后不得再提交请求。

完整性由精确计数判定：为每个关键词与目录先取一次精确总数，分页在"已收到的唯一 `fid` 数等于总数"时立即结束；
总数不可得（`ANYTXT_EXACT_COUNT=false` 或计数请求被预算拒绝）时退回"短页即结束"的启发式，
并保持与之前相同的结论口径。目录本身不可检索（`errno != 0`）时该目录记为 `scope_errno_<n>`，
其他目录的候选照常保留，整体不得标记为完整。
