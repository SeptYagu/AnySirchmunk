...
- 接口基线采用已验证的 1.3.3541 v1 配置；未知版本或模式不得凭普通查询的零结果切换参数。
...
- 以 AnyTXT 1.3.3541+ 的 v1 API 为接口基线。仅使用 `getText` 做多词字面短语的候选验证与片段展示，不把它当作最终证据；不依赖 `getFragmentAll`、`ocr`、`syncIndex` 或 MCP。
