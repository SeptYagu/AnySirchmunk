...
1. 用户直接提出问题；默认搜索 AnyTXT 已建立的全局索引，也可以明确限定一个或多个目录。
...
- 支持单个关键词和多个关键词。
- 默认搜索 AnyTXT 已建立的全局索引，不要求用户预先维护 Sirchmunk 搜索目录列表。
...
- 全局搜索优先采用本机 1.3.2477 已验证的空 `filterDir`。
- 若目标版本行为不同，可以探测 `"*"` 等兼容形式，但不得把正常零结果误判为兼容失败。
- 通过受控查询探测正则能力；记录是否可用以及是否需要回退。
...
8. 未指定路径时使用 AnyTXT 全局索引；显式指定路径时只返回该范围内的文件。
9. 在本机 1.3.2477 中使用空 `filterDir` 完成全局搜索，不依赖无效的 `"*"` 写法。
